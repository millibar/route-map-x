...
test('initNodes 今池, 3', () => {
    ...
    const expected = [
        ...
        { name: '今池', line: '東山線（高畑行）', shortestPath: ['今池'], shortestTime: 4 },
        { name: '今池', line: '東山線（藤が丘行）', shortestPath: ['今池'], shortestTime: 7 },
...
test.each([
    ...
    [{ name: '伏見', line: '鶴舞線（上小田井行）', shortestPath: ['伏見'], shortestTime: 1 },
    ...
    [{ name: '伏見', line: '鶴舞線（上小田井行）', shortestPath: ['伏見'], shortestTime: 1 },
     ...
     [{ name: '大須観音', line: '鶴舞線（上小田井行）', shortestPath: ['伏見'], shortestTime: 1 },
...
test('0. update 伏見 鶴舞線（上小田井行）', () => {
    const p = { name: '伏見', line: '鶴舞線（上小田井行）', shortestPath: ['伏見'], shortestTime: 1 };
    const V = [
        { name: '伏見', line: '東山線（高畑行）', shortestPath: ['伏見'], shortestTime: 2 },
        { name: '名古屋', line: '東山線（高畑行）', shortestPath: [], shortestTime: Infinity },
    ...
    const expected = [
        ...
        { name: '名古屋', line: '桜通線（徳重行）', shortestPath: [], shortestTime: Infinity },
        { name: '国際センター', line: '桜通線（徳重行）', shortestPath: [], shortestTime: Infinity },
        { name: '丸の内', line: '鶴舞線（上小田井行）', shortestPath: ['丸の内','伏見'], shortestTime: 2 },
        { name: '丸の内', line: '桜通線（中村区役所行）', shortestPath: [], shortestTime: Infinity }
    ];
...
test('dijkstraMain 伏見 → 国際センター', () => {
    ...
    const expected = [
        ...
        { name: '伏見', line: '東山線（高畑行）', shortestPath: ['伏見'], shortestTime: 2 },
        { name: '丸の内', line: '鶴舞線（上小田井行）', shortestPath: ['丸の内','伏見'], shortestTime: 2 },
        { name: '名古屋', line: '東山線（高畑行）', shortestPath: ['名古屋','伏見'], shortestTime: 5 },
        { name: '丸の内', line: '桜通線（中村区役所行）', shortestPath: ['丸の内','伏見'], shortestTime: 8 },
        { name: '国際センター', line: '桜通線（中村区役所行）', shortestPath: ['国際センター','丸の内','伏見'], shortestTime: 10 },
        { name: '名古屋', line: '桜通線（徳重行）', shortestPath: ['名古屋','伏見'], shortestTime: 12 },
        { name: '国際センター', line: '桜通線（徳重行）', shortestPath: ['国際センター','名古屋','伏見'], shortestTime: 13 }
    ];
    expect(actual).toStrictEqual(expected);
});
...
test('1. dijkstra 鶴舞 → 千種 0', () => {
    expect(dijkstra('鶴舞', 0, '千種', schedule)).toStrictEqual(
        { name: '千種', line: '東山線（藤が丘行）', shortestPath: ['千種','新栄町','栄','伏見','大須観音','上前津','鶴舞'], shortestTime: 25 }
    );
});

test('2. dijkstra 荒畑 → 今池 10', () => {
    expect(dijkstra('荒畑', 10, '今池', schedule)).toStrictEqual(
        { name: '今池', line: '桜通線（中村区役所行）', shortestPath: ['今池','吹上','御器所','荒畑'], shortestTime: 30 }
    );
});

test('3. dijkstra 荒畑 → 今池 11', () => {
    expect(dijkstra('荒畑', 11, '今池', schedule)).toStrictEqual(
        { name: '今池', line: '東山線（藤が丘行）', shortestPath: ['今池','千種','新栄町','栄','伏見','大須観音','上前津','鶴舞','荒畑'], shortestTime: 37 }
    );
});